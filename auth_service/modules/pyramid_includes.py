...
def ordered_include_load(stream, Loader=yaml.Loader, object_pairs_hook=OrderedDict):
    """
    This is to preserve ordering in the swagger definitions, which is important in the
    add_swagger_routing function.
    """

    class OrderedIncludeLoader(Loader):
        pass
    ...
    def extract_file(filename):
        ...
        with open(file_path, 'r') as f:
            return yaml.load(f, OrderedIncludeLoader)
    ...
    OrderedIncludeLoader.add_constructor(
        yaml.resolver.BaseResolver.DEFAULT_MAPPING_TAG,
        construct_mapping)
    OrderedIncludeLoader.add_constructor('!include', include)
    OrderedIncludeLoader.add_constructor('!import', include)
    return yaml.load(stream, OrderedIncludeLoader)
...
def server_error(exc, request):
    ...
    message = getattr(exc, 'message', "Service Error")
    logger.info("error {1} on url: {0}".format(request.url, status))
    if status >= 500:
